...
<head>
  ...
  <title>Manage Users | CNAS Admin</title>
  <style>
    body {
    ...
    .main { padding: 40px; }
    .main h2 { font-size: 28px; color: #1e40af; margin-bottom: 24px; }
...
<body>
  ...
  <nav>
    ...
    <ul>
      ...
      <li><a href="admin_users.php" class="text-blue-700">Manage Users</a></li>
...
HTML;
        foreach ($this->users as $user) {
            echo "<tr><td>{$user['id']}</td><td>{$user['username']}</td><td>{$user['role']}</td><td><button class=\"link-edit\" data-id=\"{$user['id']}\" data-username=\"{$user['username']}\" data-role=\"{$user['role']}\">Edit</button><button class=\"link-delete\" data-id=\"{$user['id']}\">Delete</button></td></tr>";
        }
        echo <<<HTML
        </tbody>
      </table>
  ...
  <script>
    ...
    form.addEventListener("submit", async e => {
      ...
      const url = "admin_users.php";
      const payload = id ? { edit: true, id, username, role } : { add: true, username, password, role };
  ...
  // Close modal
    ...
    searchInput.addEventListener("input", () => {
      ...
      for (const row of userTable.getElementsByTagName("tr")) {
